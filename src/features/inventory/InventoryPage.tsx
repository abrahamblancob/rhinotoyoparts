...
export function InventoryPage() {
  ...
  const fetchSummaries = useCallback(() => getOrgSummaries(), []);
  const nav = useAggregatorNav<OrgInventorySummary>(fetchSummaries, isPlatform);
  ...
  if (nav.navState === 'aggregators') {
    ...
    const totalStockAll = nav.summaries.reduce((s, o) => s + o.totalStock, 0);

    return (
      <OrgSelectorGrid<OrgInventorySummary>
        ...
        globalStats={[
          { title: 'Inventario Total', value: totalProducts.toLocaleString(), icon: '📦', color: '#6366F1' },
          { title: 'Stock Total', value: totalStockAll.toLocaleString(), icon: '🏷️', color: '#10B981' },
          { title: 'Agregadores', value: nav.summaries.length, icon: '🏢', color: '#8B5CF6' },
        ]}
        statFields={[
          { key: 'productCount', label: 'Productos', color: '#6366F1' },
          { key: 'totalStock', label: 'Stock Total', color: '#10B981' },
          { key: 'lowStock', label: 'Stock Bajo', color: '#D3010A', highlight: true },
          { key: 'orderCount', label: 'Órdenes', color: '#F59E0B' },
        ]}
      />
    );
  }
  ...
  return (
    <div>
      ...
      {brandData.length > 0 && (
        <div className="rh-card mb-6" style={{ padding: '20px 24px' }}>
          <h3 className="rh-card-title" style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
            📊 Productos por Marca
            <span style={{ fontSize: 12, fontWeight: 400, color: '#94A3B8' }}>(Top {brandData.length})</span>
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {brandData.map(([brand, count]) => (
              <div key={brand} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ width: 100, fontSize: 13, fontWeight: 600, color: '#1E293B', textAlign: 'right', flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={brand}>{brand}</span>
                <div style={{ flex: 1, backgroundColor: '#F1F5F9', borderRadius: 6, height: 24, overflow: 'hidden' }}>
                  <div style={{ width: `${(count / maxBrandCount) * 100}%`, height: '100%', backgroundColor: '#10B981', borderRadius: 6, minWidth: 2, transition: 'width 0.3s ease' }} />
                </div>
                <span style={{ width: 40, fontSize: 13, fontWeight: 700, color: '#10B981', textAlign: 'right', flexShrink: 0 }}>{count}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="rh-filters" style={{ marginTop: 24 }}>
        <input type="text" placeholder="Buscar por nombre, SKU o OEM..." value={search}
          onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} className="rh-search" />
        <div className="flex gap-2">
          {[
            ...
            <button key={f.key} onClick={() => { setStatusFilter(f.key); setCurrentPage(1); }}
              className={`rh-filter-pill ${statusFilter === f.key ? 'active' : ''}`}>{f.label}</button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState icon="📦" title="No hay productos" description="Agrega productos a tu inventario para comenzar a gestionar"
        ...
        <div className="rh-table-wrapper">
              ...
              {paginatedProducts.map((product) => (
                <tr key={product.id} className="cursor-pointer" onClick={() => canWrite('inventory') && openEditModal(product)}>
                  ...
                  <td className="text-right">
                    <span className="font-semibold" style={{ color: product.stock === 0 ? '#D3010A' : product.stock <= product.min_stock ? '#F59E0B' : '#10B981' }}>
                      {product.stock}
                    </span>
                  </td>
                  ...
                  {canWrite('inventory') && (
                    <td className="text-center">
                      <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(product); }}
                        className="rh-btn rh-btn-ghost" style={{ color: '#D3010A', padding: '4px 8px', fontSize: 13 }} title="Eliminar producto">🗑️</button>
                    </td>
                  )}
                </tr>
              ))}
          ...
          <Pagination currentPage={currentPage} totalPages={totalPages} totalItems={sortedProducts.length} pageSize={PAGE_SIZE} onPageChange={setCurrentPage} />
        </div>
      )}
      ...
      <ProductFormModal
        ...
        orgId={isPlatform ? orgId : organization?.id}
        isPlatform={isPlatform}
        onClose={() => { setShowProduct(false); setEditingProduct(null); }}
        onSaved={loadProducts}
        onRequestDelete={setDeleteTarget}
        canDelete={canWrite('inventory')}
      />

      <ConfirmDeleteModal open={!!deleteTarget} title="Eliminar Producto" loading={deleteLoading}
        onClose={() => setDeleteTarget(null)} onConfirm={handleDeleteProduct}>
        <p style={{ fontWeight: 500, marginBottom: 8 }}>¿Estas seguro de que deseas eliminar este producto?</p>
        {deleteTarget && <p style={{ color: '#94A3B8', fontSize: 14 }}><strong>{deleteTarget.name}</strong> (SKU: {deleteTarget.sku})</p>}
        <p style={{ color: '#F59E0B', fontSize: 13, marginTop: 12 }}>Esta accion no se puede deshacer.</p>
      </ConfirmDeleteModal>
    </div>
  );
}

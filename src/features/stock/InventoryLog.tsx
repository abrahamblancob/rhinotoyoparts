...
export function InventoryLog({ orgId, warehouseId, includeChildren }: InventoryLogProps) {
  ...
  const { data: snapshots, loading } = useAsyncData<InventorySnapshot[]>(fetcher, [orgId, warehouseId, includeChildren]);

  const items = snapshots ?? [];

  if (loading) {
    return <p style={{ textAlign: 'center', color: '#94A3B8', padding: 24 }}>Cargando historial...</p>;
  }
